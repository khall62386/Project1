#!/bin/bash

#Check if script was run as root. Wxit if false

if [ $UID -ne 0 ]
then
 echo "Please run this script with sudo."
 exit
fi

#Check if script was run as root. Exit if false.
if [ $UID -ne 0 ]
then
  echo "Please run this script with sudo."
  exit
fi

# Define Variables
output=$HOME/research/sys_info.txt
ip=$(ip addr | grep inet | tail -2 | head -1)
execs=$(find /home -type f -perm 777 2> /dev/null)


# Check for research directory. Create it if needed.
if [ ! -d $HOME/research ]
then
 mkdir $HOME/research
fi

# Check for output file. Clear it if needed.
if [ -f $output ]
then
  rm $output
fi

# Start of script
# System Audit
echo "A Quick System Audit Script" >> $output
date >> $output
echo "" >> $output

# Machine Type and Info
echo "Machine Type Info:" >> $output
echo -e "$MACHTYPE \n" >> $output

# Name  Whoami
echo -e "Uname info: $(uname -a) \n" >> $output

# IP information
echo -e "IP Info:" >> $output
echo -e "$ip \n" >> $output

# Name of Host
echo -e "Hostname: $(hostname -s) \n" >> $output

# DNS Servers
echo "DNS Servers: " >> $output

# Resolved Conf
cat /etc/resolv.conf >> $output

# Memory Info
echo -e "\nMemory Info:" >> $output
free >> $output

# CPU Info
echo -e "\nCPU Info:" >> $output
lscpu | grep CPU >> $output

# Disk Space
echo -e "\nDisk Usage:" >> $output
df -H | head -2 >> $output

# Who logged in
echo -e "\nWho is logged in: \n $(who -a) \n" >> $output

# Nexec Files
echo -e "\nexec Files:" >> $output

# Execs
echo $execs >> $output

# Top Processes
echo -e "\nTop 10 Processes" >> $output
ps aux --sort -%mem | awk {'print $1, $2, $3, $4, $11'} | head >> $output

# 1 Put the paths of the shadow and passwd files (from the /etc directory) in a list.

files=('/etc/passwd' '/etc/shadow')

# 2 Create a for loop that prints out the permissions of each file in your file list

echo -e "\nThe permissions for sensitive /etc files: >> $output"
 for file in ${files[@]}
  do
   ls-l $file >> $output
done

# 3 Put the final touches on your script by adding comments for each action your script completes

#Creating names for developers can understand your code.
#Display CPU usage
echo -e "\nCPU Info:" >> $output
lscpu | grep CPU >> $output

# Bonus 1 
#1 Create a for loop that checks the sudo abilities of each user who has a home directory

for user in $(ls /home); do sudo -lU $user; done

# Bonus 2
# Create a list that contains the commands date, uname -a, and hostname -s

# Remove the lines that use these commands and replace them with a for loop that prints out The results of the command are:  along with the results of running the command.

for x in {0..2}
do
 results=$(${commands[$x]})
 echo "Results of \"${commands[$x]}\" command:"
 echo $results
 echo ""

done
